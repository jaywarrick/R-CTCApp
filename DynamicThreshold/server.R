rm(list=ls())
source("C:/Users/Mengcheng/Documents/GitHub/R-CTCApp/DynamicThreshold/CTCApp_HelperFunctions.R")
library(foreign)
...
shinyServer(function(input, output) {
      ...
      temp0 <- calculateSummaryVariables(locResults=locResults, dapiColor=1, ckColor=2, cd45Color=3, arColor=4)
  
     output$distPlot <- renderPlot({
       
         temp <- calculateSummaryVariables_Threshold(locResults=temp0, CKThresh_SD=input$CK_SD, DAPIThresh_SD=input$DPAI_SD, CD45Thresh_SD=input$CD45_SD, ARThresh_SD=input$AR_SD)
         ...
         output$xSlider <- renderUI({
           if (input$x1cursor){
              minmax_x <- range(nonCTCs[,input$x1])
              sliderInput(inputId="x1Range",label="X", min=floor(minmax_x[1]), max=ceiling(minmax_x[2]), value=c(minmax_x[1],minmax_x[2]))
            }
         })
     
         output$ySlider <- renderUI({
           if (input$y1cursor){
             minmax_y <- range(nonCTCs[,input$y1])
             sliderInput(inputId="y1Range",label="Y", min=floor(minmax_y[1]), max=ceiling(minmax_y[2]), value=c(minmax_y[1],minmax_y[2]))
           }
         })
